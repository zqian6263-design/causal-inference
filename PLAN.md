...
> 📌 **状态说明（2026-09-22）**：本文档是项目的**顶层任务书**，保留的是最初的设计意图与验收标准。**Phase 0–4 已全部执行完毕**，后续又经过优化批次 A–J 的多轮加固，因此文中个别路径/目录描述可能已与现状有出入。**仓库当前状态、最新实证结论与选型建议，以 [`README.md`](README.md) 和《[因果推断方法体系与实证总览](因果推断方法体系与实证总览.md)》为准。** 逐轮执行记录见 [`REPORT.md`](REPORT.md)。
...
**seed 纪律**：数据生成固定 seed=42；方法内部随机性用 `random.seed(seed)` 固定——**注意 GRaSP/BOSS 走的是 Python 标准库全局 `random`，`np.random.seed` 对它们无效**；凡有内部随机性的方法一律报 5 seed（42/1/7/2024/999）mean±std，禁止调 seed 刷指标。
