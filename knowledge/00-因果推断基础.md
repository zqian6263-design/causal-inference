...
- `causal-learn/docs/source/getting_started.rst`
...
- `causal-learn/docs/source/independence_tests_index/index.rst`（独立性检验总览）
- `causal-learn/causallearn/utils/cit.py`（CIT 实现；`d_separation` 类依赖 nx ≥ 3.3）

> `causal-learn/` 为仓库同级目录的官方源码克隆（可选，仅供阅读 API）；实验用 pip 版 causal-learn 0.1.4.8 即可。

## ✅ 验证输出

本文两个代码示例（d-分离 `d_separated`、等价类 PC 演示）已在本机实跑通过（causal-learn 0.1.4.8，networkx 3.2.1，版本见 `requirements.txt`）。运行输出见上文代码块后的「运行输出」。完整脚本：`experiments/00_env_check.py`。
