...
- `causal-learn/docs/source/search_methods_index/Constraint-based causal discovery methods/PC.rst`
- `causal-learn/docs/source/search_methods_index/Constraint-based causal discovery methods/FCI.rst`
- `causal-learn/docs/source/search_methods_index/Constraint-based causal discovery methods/CDNOD.rst`
- `causal-learn/docs/source/independence_tests_index/fisherz.rst` / `chisq.rst` / `gsq.rst` / `kci.rst` / `mvfisherz.rst`
- `causal-learn/causallearn/search/ConstraintBased/PC.py`、`FCI.py`、`CDNOD.py`（源码，参数与边属性权威定义）
- `causal-learn/tests/TestPC.py`、`TestFCI.py`、`TestCDNOD.py`（官方学习用例与基准图）
...
> ⚠️ 环境注意：官方 `tests/TestFCI.py` 里的 `d_separation` 检验在本环境需先 `import scripts.patch_d_separation` 启用（networkx 3.2.1 缺 `is_d_separator`，补丁用等价的 `d_separated` 补齐，已验证可用）；上述示例一律用真实数据 + `fisherz / kci / mv_fisherz`。
